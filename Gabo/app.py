...
if 'macros' not in st.session_state:
    st.session_state.macros = {"calorias": 2000, "proteina": 150, "carbos": 200, "grasas": 65}
...
def mostrar_app_principal():
    ...
    if menu == "📊 Mi Perfil y Macros":
        ...
        with st.expander("🧮 ¿No sabes tus macros? Calcúlalos aquí", expanded=True):
            ...
            with col2:
                altura = st.number_input("Altura (cm)", min_value=100.0, max_value=250.0, value=175.0)
        ...
        st.subheader("Tus Macros Actuales (Los usará el motor)")
                ...
                for i, tab in enumerate(tabs):
                    ...
                    with tab:
                        ...
                        if data['costo'] > presupuesto:
                            st.warning("⚠️ Esta opción supera tu presupuesto.")
                        else:
    ...
    # ---------------------------------------------------------
        ...
        st.sidebar.markdown("---")
        st.sidebar.subheader("⚙️ Panel de Demo (Para Jueces)")
        ...
        if st.button("📸 Escanear Plato con Hardware", use_container_width=True):
            ...
            with st.spinner("Inicializando cámara de la Raspberry Pi 5..."):
                ...
                try:
                    ...
                    st.warning("⚠️ Módulo de cámara Raspberry no detectado (¿Estás en Windows?). Usando imagen de simulación.")
            ...
            if foto_path and os.path.exists(foto_path):
                ...
                st.info("🖼️ [Imagen simulada de un pedazo de pizza]")
            ...
            if plan["cambiar_lista_compras"]:
                st.error("⚠️ Tu despensa actual ya no cuadra con tus objetivos.")
            else:
                col1, col2 = st.columns(2)
